...
const App = {
    ...
    events() {
        ...
        const onMove = (e) => {
            ...
            // Ne pas bloquer le scroll sur les menus
            if (e.target.closest('.side-bar') || e.target.closest('#library-panel') || e.target.closest('.modal')) return;
            ...
            if (e.cancelable) e.preventDefault(); // Bloque le scroll uniquement sur le jeu
        ...
        // Application ciblée des événements
...
const UI = {
    ...
    updateScenesList() {
        ...
        State.scenes.forEach((s, i) => {
            ...
            d.innerHTML = `<span>${s.name}</span><button onclick="IO.deleteScene(${s.id}, event)">✕</button>`;
